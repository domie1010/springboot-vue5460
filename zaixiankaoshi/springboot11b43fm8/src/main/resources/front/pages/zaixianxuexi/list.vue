...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","padding":"24rpx 0 24rpx","alignItems":"flex-start","flexWrap":"wrap","background":"#f0f8fa","display":"block","width":"100%","position":"relative","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"border":"0px solid #ff9593","padding":"0","margin":"0 auto","borderRadius":"0px","background":"none","display":"flex","width":"calc(100% - 48rpx)","height":"auto"}'>
				...
				<view :style='{"margin":"0 0px","borderColor":"#ff9593","borderRadius":"0","flex":"1","background":"#fff","borderWidth":"0","lineHeight":"80rpx","position":"relative","borderStyle":"solid","height":"80rpx"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#666","borderRadius":"0","background":"none","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.shipinbiaoti" type="text" placeholder="视频标题" ></input>
				</view>
			...
			<view class="list" :style='{"padding":"32rpx 24rpx 24rpx","margin":"0px auto 0","borderRadius":"0px","background":"#ffffff","flex":"1","width":"calc(100% - 200rpx)","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"border":"0px solid #eee","padding":"0px","margin":"0 0 40rpx","borderRadius":"12rpx","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"200rpx","objectFit":"cover","borderRadius":"8rpx","display":"block","height":"240rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.shipintupian)" :src="product.shipintupian.split(',')[0]"></image>
					<image :style='{"width":"200rpx","objectFit":"cover","borderRadius":"8rpx","display":"block","height":"240rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.shipintupian?baseUrl+product.shipintupian.split(',')[0]:''"></image>
					...
					<view class="listmr" :style='{"padding":"0 0 0 20rpx","margin":"0","alignItems":"center","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20240221/8e2fddf1cb4644f6b32d79a4137c914d.png) no-repeat right bottom / 48rpx","display":"flex","width":"calc(100% - 200rpx)","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>视频标题:{{product.shipinbiaoti}}</view>
					...
					<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zaixianxuexi','修改')) || (!userid && isAuthFront('zaixianxuexi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zaixianxuexi','删除')) || (!userid && isAuthFront('zaixianxuexi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zaixianxuexi','修改')) || (!userid && isAuthFront('zaixianxuexi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zaixianxuexi','删除')) || (!userid && isAuthFront('zaixianxuexi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
		...
		<button :style='{"border":"0","boxShadow":"none","color":"#333","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#ffcc00","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('zaixianxuexi','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"none","color":"#333","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#ffcc00","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('zaixianxuexi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`zaixianxuexi`, params);
                } else {
                    res = await this.$api.list(`zaixianxuexi`, params);
                }
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`zaixianxuexi`, searchForm);
                } else {
                    res = await this.$api.list(`zaixianxuexi`, searchForm);
                }
				// 如果是第一页数据置空
