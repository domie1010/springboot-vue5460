...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
			...
			<view class="list" :style='{"padding":"32rpx 24rpx 24rpx","margin":"0px auto 0","borderRadius":"0px","background":"#ffffff","flex":"1","width":"calc(100% - 200rpx)","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"border":"0px solid #eee","padding":"0px","margin":"0 0 40rpx","borderRadius":"12rpx","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusszaixianxuexi','修改')) || (!userid && isAuthFront('discusszaixianxuexi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusszaixianxuexi','删除')) || (!userid && isAuthFront('discusszaixianxuexi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusszaixianxuexi','修改')) || (!userid && isAuthFront('discusszaixianxuexi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusszaixianxuexi','删除')) || (!userid && isAuthFront('discusszaixianxuexi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
		...
		<button :style='{"border":"0","boxShadow":"none","color":"#333","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#ffcc00","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('discusszaixianxuexi','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"none","color":"#333","bottom":"120rpx","right":"120rpx","borderRadius":"100%","background":"#ffcc00","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('discusszaixianxuexi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discusszaixianxuexi`, params);
                } else {
                    res = await this.$api.list(`discusszaixianxuexi`, params);
                }
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discusszaixianxuexi`, searchForm);
                } else {
                    res = await this.$api.list(`discusszaixianxuexi`, searchForm);
                }
				// 如果是第一页数据置空
